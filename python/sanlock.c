...
for waiting for lockspace events using poll. When the file descriptor becomes\n\
readble, you can use get_event to get pending events. When you are done, you\n\
must unregister the event listener using end_event.");
...
py_get_event(PyObject *self __unused, PyObject *args)
{
    ...
    for (;;) {
        ...
        /* from_host_id */
        if ((value = PyInt_FromLong(from_host_id)) == NULL)
            goto exit_fail;
        ...
        /* from_generation */
        if ((value = PyInt_FromLong(from_generation)) == NULL)
            goto exit_fail;
        ...
        /* host_id */
        if ((value = PyInt_FromLong(he.host_id)) == NULL)
            goto exit_fail;
        ...
        /* generation */
        if ((value = PyInt_FromLong(he.generation)) == NULL)
            goto exit_fail;
        ...
        /* event */
        if ((value = PyInt_FromLong(he.event)) == NULL)
            goto exit_fail;
        ...
        /* data */
        if ((value = PyInt_FromLong(he.data)) == NULL)
            goto exit_fail;
...
set_event(lockspace, host_id, event, data=0, generation=0, flags=0)\n\
...
Arguments\n\
  ...
  event             event number (int)\n\
  data              optional event data (int)\n\
  generation        optional recipient generation (int)\n\
  flags             optional combination of event flags (int)\n\
\n\
Flags\n\
  SETEV_CUR_GENERATIION     if generation is zero, use current host\n\
                            generation.\n\
...
\n\
  ...
  set_event('foo', 7, 1, flags=SETEV_CUR_GENERATIION)\n\
\n\
  Send event 3 to all hosts on lockspace 'foo', replacing previous events\n\
  sent to other hosts.\n\
  set_event('foo', 0, 3, flags=SETEV_ALL_HOSTS|SETEV_REPLACE_EVENT)\n\
\n\
Notes\n\
\n\
...
py_set_event(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    static char *kwlist[] = {"lockspace", "host_id", "event", "data",
                             "generation", "flags", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, keywds, "skL|LLi", kwlist,
        &lockspace, &he.host_id, &he.event, &he.data, &he.generation,
        &flags)) {
        return NULL;
    }
...
#define PYSNLK_INIT_ADD_CONSTANT(x, y) \
    ...
    /* set event flags */
    PYSNLK_INIT_ADD_CONSTANT(SANLK_SETEV_CUR_GENERATION, "SETEV_CUR_GENERATIION");
